...
class Bottleneck(nn.Module):
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv3 = nn.Conv2d(planes, 3 * planes, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(3 * planes)
        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != 3 * planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, 3 * planes, kernel_size=1, stride=stride),
                nn.BatchNorm2d(3 * planes)
            )
...
class TripleNet(nn.Module):
    def __init__(self):
        ...
        ch_list = [  128, 192, 256, 320, 1080]
        gr       = [  32, 16, 20, 40,160]
        n_layers = [   6, 16, 16, 16,  3]
        downSamp = [   1,  0,  1,  1,  0]
        ...
        for i in range (96, 256, 32):
          self.base.append ( DenseLayer(i, 32, kernel=3, stride=1,  bias=False) )      
