...
class ConvLayer_1(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel=3, stride=1, bias=False):
        super(ConvLayer_1, self).__init__()
    ...
    def forward(self, x):
        new_features = super(ConvLayer_1, self).forward(x)
...
class ConvLayer_3(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        super(ConvLayer_3, self).__init__()
...
class TripleNet(nn.Module):
    def __init__(self, pretrained=True, weight_path=''):
        ...
        # First Block
        self.base.append ( ConvLayer_1(ch, 32, kernel=3, stride=1,  bias=False) ) 
        self.base.append ( ConvLayer_1(96, 32, kernel=3, stride=1,  bias=False) )
        self.base.append ( ConvLayer_1(128, 32, kernel=3, stride=1,  bias=False) )
        self.base.append ( ConvLayer_1(160, 32, kernel=3, stride=1,  bias=False) )
        self.base.append ( ConvLayer_1(192, 32, kernel=3, stride=1,  bias=False) )
        self.base.append ( ConvLayer_1(224, 32, kernel=3, stride=1,  bias=False) )        
        ...
        # Second Block
        blk = ConvLayer_2(ch, gr[1], grmul, n_layers[1])
        ch = blk.get_out_ch()
        ...
        # Third Block
        blk = ConvLayer_2(ch, gr[2], grmul, n_layers[2])
        ch = blk.get_out_ch()
        self.base.append ( blk )
        ...
        # Fourth Block
        blk = ConvLayer_2(ch, gr[3], grmul, n_layers[3])
        ch = blk.get_out_ch()
        self.base.append ( blk )
        ...
        # Fifth Block     
        self.base.append ( ConvLayer_3(ch, 160))
        self.base.append ( ConvLayer_3(480, 240))
        ...
        if pretrained:
          weight_file = '%striplenet.pth'%(weight_path)            
...
def triplenet():
    return TripleNet(pretrained=False, weight_path='D:\\pre-trained-cifar10\\')